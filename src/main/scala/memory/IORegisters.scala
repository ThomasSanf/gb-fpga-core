...
class IORegistersIO extends Bundle {
  ...
  val ppuLy   = Input(UInt(8.W))   // FF44 - LY (READ FROM PPU!)
...
class IORegisters extends Module {
  ...
  switch(io.addr) {
    ...
    is(0x44.U) { readData := io.ppuLy }  // Read from PPU!
  ...
  when(io.write) {
    switch(io.addr) {
      ...
      is(0x02.U) {
        ...
        when(io.writeData(7)) {
          //printf("[SERIAL TRIGGERED] SB=%x SC=%x\n", regSB, io.writeData)
          //printf("%c", regSB)
          serialOutReg := regSB
          serialValidReg := true.B
          regSC := io.writeData & 0x7F.U  // Clear bit 7 IMMEDIATELY
        }.otherwise {
          regSC := io.writeData  // Normal write when bit 7 not set
        }
      }
