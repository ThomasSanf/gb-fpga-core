...
class GameBoySoC(romSize: Int, romPath: String) extends Module {
  val io = IO(new Bundle {
    ...
    // PPU Timing signals for framebuffer synchronization
  ...
  // Expose timing signals for framebuffer synchronization
  io.hblank     := ppu.io.hblank
  io.vblank     := ppu.io.vblank
  io.lcdEnable  := ppu.io.lcdEnable
